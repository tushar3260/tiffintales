...
import { FaCalendarAlt, FaCheckCircle, FaTimesCircle, FaRedoAlt } from "react-icons/fa";
...
export default function SubscriptionPage() {
  ...
  useEffect(() => {
    const fetchSubscription = async () => {
      if (!user?._id) return;
      try {
        // ✅ FIXED: Use VITE_API_URL + correct route /subscriptions/user/:userId
  ...
  if (!subscriptions.length)
    return (
      <div className="p-6 sm:p-10 text-center min-h-screen bg-gradient-to-b from-orange-50 to-white flex flex-col items-center justify-center gap-6">
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-10 bg-gradient-to-b from-orange-50 to-white min-h-screen">
      ...
      <div className="space-y-4">
        {subscriptions.map((sub, idx) => (
          <motion.div
            ...
            className="bg-white rounded-2xl shadow-md p-5 border border-orange-100 hover:shadow-lg transition"
          >
            ...
            <div className="flex gap-3 mt-4 flex-wrap">
              {sub.status === "Active" && (
                <button className="flex items-center gap-1 px-4 py-2 bg-orange-500 text-white rounded-lg text-sm hover:bg-orange-600 transition">
                  <FaRedoAlt /> Renew
                </button>
              )}
              <button
                onClick={() => navigate("/meals")}
                className="flex items-center gap-1 px-4 py-2 border border-orange-300 text-orange-600 rounded-lg text-sm hover:bg-orange-50 transition"
              >
                Browse Meals
              </button>
            </div>
